...
export const VideoPlayer: React.FC<VideoPlayerProps> = ({ title, url, onClose }) => {
  ...
  const playWithMpegts = (streamUrl: string) => {
    ...
    try {
      console.log('🔍 Checking if mpegts is available:', !!(window as any).mpegts);
      
      if (!(window as any).mpegts) {
        console.warn('⚠️  mpegts.js not loaded, trying HLS fallback');
        playWithHLS(streamUrl);
        return;
      }
      
      const mpegts = (window as any).mpegts;
      console.log('📡 Creating MPEGTS player with URL:', streamUrl);
      ...
      console.log('🚀 MPEGTS player loading stream...');
      player.load();
      player.play().catch((e: any) => {
        console.error('❌ Play error:', e);
      });
      console.log('✅ MPEGTS player started');
    } catch (err) {
      console.error('❌ MPEGTS error:', err);
      console.log('⚠️  Falling back to HLS');
      playWithHLS(streamUrl);
    }
  };
